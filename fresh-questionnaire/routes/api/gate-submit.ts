...
export const handler: Handlers = {
  async POST(req, _ctx) {
    ...
    try {
      ...
      console.log('[gate-submit] Magic link created, expires:', expiresAt.toISOString());
      ...
      return new Response(
        JSON.stringify({
          ...
          ...(Deno.env.get('DENO_ENV') !== 'production' && {
            ...
            _devGateToken: gateToken,
          }),
        }),
        { status: 200, headers: { 'Content-Type': 'application/json' } }
      );
    } catch {
      console.error('[gate-submit] Failed');
