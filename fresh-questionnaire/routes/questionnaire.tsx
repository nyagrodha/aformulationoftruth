...
export const handler: Handlers<QuestionnaireData> = {
  async GET(req, ctx) {
    try {
      ...
      // If no session id, try to find one in cookies
      if (!sessionId) {
        const sessionMatch = incomingCookie.match(/questionnaireSessionId=([^;]+)/);
        sessionId = sessionMatch ? sessionMatch[1] : null;
      }

      // If still no session id, create one via API (requires auth)
      const setCookieHeaders: string[] = [];
      if (!sessionId) {
        const sessionResponse = await fetch(`${apiUrl}/api/questionnaire/session`, {
          method: "GET",
        ...
        if (!sessionResponse.ok) {
          throw new Error(`Failed to initialize questionnaire session: ${sessionResponse.statusText}`);
        }

        const session = await sessionResponse.json();
        sessionId = session.id;
        setCookieHeaders.push(`questionnaireSessionId=${session.id}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=604800`);
      }
      ...
      const questionResponse = await fetch(`${apiUrl}/api/questionnaire/${sessionId}/current`, {
        headers: incomingCookie ? { cookie: incomingCookie } : undefined,
      });
      ...
      setCookieHeaders.forEach((cookie) => resp.headers.append("Set-Cookie", cookie));
      return resp;
    } catch (error) {
...
export default function QuestionnairePage({ data }: PageProps<QuestionnaireData>) {
  ...
  const { question, progress, sessionId: sid } = data;
  const sessionId = sid!;
