...
review of the first draft; sections marked _Resolved during build_ record what
measurement changed.
...
darkened before use — see _Colour_ below.
...
| Assumption                             | Reality                                                                                                                                                                                                 |
| -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A hamburger exists and needs restyling | There is **no hamburger**. `islands/Nav.tsx` makes the _wordmark itself_ the disclosure toggle. Adding the mark changes the interaction model, not just the paint.                                      |
...
| The palette maps onto the wordmark     | The wordmark has ~3 colour slots (`a`+`सत्य` share magenta, `4` blue, `முல` gold). Five colours map onto five _bars_, not onto four glyphs.                                                              |
...
- `.nav-arrow` (the `▶` that rotated on open) is **removed**. The mark itself
  carries the open/closed state instead — see _Open state_ below.
...
under it — on cream the cyan is close to invisible. This is the _same_ failure
...
real destination when the encrypted-messaging feature lands — see _Out of scope_.
...
- **The mask is _referenced_ by bars 2–4**, and by neither bar 1 nor bar 5.
...
test asserting only that the mask _exists_ would pass on a mark rendering five
...
2. `prolegomenon.css` carried `@media (width <= 900px) { .site-nav
   ...
   wordmark became an `<a>` it matched _that_, **deleting the wordmark on every
