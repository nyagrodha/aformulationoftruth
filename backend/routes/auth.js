...
router.get('/verify', async (req, res) => {
  ...
  try {
    ...
    // Redirect to questionnaire with session token (Fisher-Yates shuffled questions)
    res.redirect(`/questionnaire.html?auth=success&token=${sessionToken}&email=${encodeURIComponent(record.email)}`);
  } catch (error) {
...
// Handle /auth/callback - same logic as /auth/verify (for email compatibility)
