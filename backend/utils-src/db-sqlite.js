...
export function saveMagicLinkToken(email, token, geolocation = null) {
  return new Promise((resolve, reject) => {
    ...
    db.run(
      'INSERT INTO magic_link_tokens (token, email, expires_at, geolocation) VALUES (?, ?, ?, ?)',
      [token, email, expiresAt, geoJSON],
      (err) => {
        if (err) {
...
export function findMagicLinkToken(token) {
  return new Promise((resolve, reject) => {
    db.get(
      'SELECT email, expires_at, geolocation FROM magic_link_tokens WHERE token = ?',
      [token],
      (err, row) => {
        if (err) {
