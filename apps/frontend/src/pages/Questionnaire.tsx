...
export default function QuestionnairePage() {
  ...
  useEffect(() => {
    // Main logic for when the question changes
  ...
  const currentQuestionId = session.questionOrder[currentQuestionIndex];
  // Placeholder: In a real app, you would fetch question text from a service
    ...
    <div className={`min-h-screen relative transition-colors duration-500 ${containerClass}`}>
      ...
      <main className="max-w-4xl mx-auto p-4 sm:p-8 pt-20">
        <QuestionCard
          ...
          declined={false}
          reviewingDeclined={false}
          onDecline={() => console.log('Decline action')}
          hasUnsavedChanges={currentAnswer !== (answers[currentQuestionId] || '')}
          isNavigating={false}
        />
