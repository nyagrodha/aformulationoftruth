...
class QuestionService {
  ...
  // Blank answers are acceptable
  if (!answer || !answer.trim()) return { isValid: true };

  const s = answer.trim();
  if (/^\d+$/.test(s)) return { isValid: false, errors: ["Answer cannot be only numbers"] };
  if (/^([a-zA-Z])\1+$/.test(s)) return { isValid: false, errors: ["Answer cannot be repeated letters only"] };
  return { isValid: true };
}
}
