...
import { decrypt, deriveKey, encrypt, hashEmail, randomToken, sha256 } from '../lib/crypto.ts';

// Helper to generate a random salt
//
// The type argument is not decoration. Since TypeScript 5.7 Uint8Array is
// generic over its backing buffer, and a bare `Uint8Array` means
// `Uint8Array<ArrayBufferLike>` -- which includes SharedArrayBuffer and so is
// NOT assignable to deriveKey's `Uint8Array<ArrayBuffer>`. This annotation was
// therefore widening a value that getRandomValues already returns at the
// correct narrow type, and produced all 12 TS2345 errors in this file from one
// line. Dropping the annotation entirely would also work; keeping it explicit
// documents the contract deriveKey (lib/crypto.ts:68) requires.
