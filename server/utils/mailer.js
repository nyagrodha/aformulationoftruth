...
try {
  ...
  smtpConfigured = true;
  console.log('✓ Apple SMTP configured for admin emails');
} catch (error) {
...
export async function sendMagicLinkEmail(email, token) {
  try {
    ...
    // TESTING MODE: No email service configured
...
export async function sendQuestionnairePDF(email, pdfPath, username) {
  try {
    ...
    console.error('Error sending questionnaire PDF email:', error.message);
