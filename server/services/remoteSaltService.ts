  ...
  async storeSalt(
    ...
    for (const serverName of serverOrder) {
      ...
      try {
        ...
        if (result.success) {
          console.log(`✅ Salt stored on ${serverName} server`);
          return { ...result, server: serverName } as any;
        }
      } catch (error) {
        console.error(`❌ Failed to store salt on ${serverName}`);
  ...
  async retrieveSalt(saltId: string, serverHint?: string): Promise<RetrieveSaltResponse> {
    ...
    for (const serverName of serverOrder) {
      ...
      try {
        ...
        console.error(`❌ Failed to retrieve salt from ${serverName}`);
        // Continue to next server
      }
    }
  ...
  async deleteSalt(saltId: string, serverHint?: string): Promise<boolean> {
    ...
    for (const config of servers) {
      try {
        ...
        console.error('Salt deletion error');
      }
    }
