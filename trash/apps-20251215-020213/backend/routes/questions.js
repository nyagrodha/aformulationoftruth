...
router.get('/next', async (req, res) => {
  // Get session ID from query parameter or use authenticated user's email
  let sessionId = req.query.session_id || req.query.sessionId;

  // If no session_id provided, use the authenticated user's email as session ID
  ...
  if (!sessionId) {
    return res.status(400).json({
      ...
      message: 'Please provide a session_id query parameter or authenticate with a JWT token'
    });
  }
