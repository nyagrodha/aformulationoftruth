 ...
 */
export async function saveIPGeolocation(dbClient, ipData) {
    try {
        // Check if IP already exists
        const existing = await dbClient.query('SELECT id FROM ip_geolocation WHERE ip_address = $1', [ipData.ip]);
        if (existing.rows.length > 0) {
            // Update last_seen and increment lookup count
            await dbClient.query(`UPDATE ip_geolocation
         SET last_seen = NOW(),
        ...
        const result = await dbClient.query(`INSERT INTO ip_geolocation (
 ...
 */
export async function recordUserIPAccess(dbClient, userId, ipId, userAgent, action) {
    try {
        await dbClient.query(`INSERT INTO user_ip_history (user_id, ip_id, user_agent, action)
       VALUES ($1, $2, $3, $4)`, [userId, ipId, userAgent, action]);
    }
    catch (error) {
 ...
 * @param ipId - IP geolocation record ID
 */
export async function updateUserCurrentIP(dbClient, userId, ipId) {
    try {
        await dbClient.query('UPDATE users SET current_ip_id = $1 WHERE id = $2', [ipId, userId]);
    }
    catch (error) {
 ...
 */
export async function getIPGeolocation(dbClient, ipAddress) {
    try {
        const result = await dbClient.query('SELECT * FROM ip_geolocation WHERE ip_address = $1', [ipAddress]);
 ...
 */
export async function getUserIPHistory(dbClient, userId, limit = 10) {
    try {
        const result = await dbClient.query(`SELECT h.*, g.ip_address, g.city, g.country, g.is_vpn, g.is_tor
 ...
 */
export async function isIPRegistered(dbClient, ipAddress, excludeEmail) {
    try {
