 ...
 */
export function deriveKeyFromToken(token: string): Buffer {
 ...
 */
export function encryptGeolocation(data: any, token: string): string {
  try {
 ...
 */
export function decryptGeolocation(encryptedData: string, token: string): any {
  try {
    const key = deriveKeyFromToken(token);
