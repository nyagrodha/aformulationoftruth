"""Dakṣiṇa Parva Anuvādam — 35 Proust Questionnaire questions in Tamil.

Questions 0–1: வாயில் (Vāyil) — The Gate (fixed order)
Questions 2–34: Main questionnaire (Feynman-shuffled per session)
"""
...
def to_tamil_numeral(n: int) -> str:
    """Convert a non-negative integer to Tamil numerals."""
    if not isinstance(n, int) or n < 0:
        raise ValueError(f"Expected non-negative integer, got: {n}")
    if n == 0:
        return TAMIL_NUMERALS[0]
    result = ""
    num = n
    while num > 0:
...
GATE_QUESTIONS: list[Question] = [q for q in QUESTIONS if q.is_gate]
MAIN_QUESTIONS: list[Question] = [q for q in QUESTIONS if not q.is_gate]
