...
import { hashResumeToken } from './crypto.ts';
...
export async function authenticateRequest(
  ...
  if (jwtToken) {
    ...
    if (payload) {
      ...
      if (session) {
        if (payload.via) return { session, via: payload.via };

        /*
         * No `via` at all: a token minted before the claim existed, and so
         * necessarily within its 24-hour validity.
         *
         * It must NOT be read as 'link'. An earlier draft of this file did, on
         * the reasoning that /auth/verify is the only route that ever sets a jwt
         * cookie -- which is true of THIS branch and false of the deployed
         * fleet: routes/api/gate-submit.ts on fix/questionnaire-funnel (pushed,
         * and reachable from origin) ends with
         * `headers.append('Set-Cookie', jwtCookie(jwt))`. A claimless token in a
         * cookie may therefore have come from someone who did nothing but type
         * an address, and reading it as proof is the exact escalation
         * resumeCookie() below was written to describe.
         *
         * Nor can it simply be read as 'gate', which would lock every current
         * respondent out of the messaging surface for a day. The resume token
         * settles it: it is delivered only by mail to the address, so a holder
         * who has one has proved what 'link' asserts. Checking it against the
         * JWT's own session id -- rather than calling getSessionByToken --
         * avoids that helper's completed_at filter, which would strip the proof
         * from anyone who had finished answering.
         */
        if (resumeToken && (await hashResumeToken(resumeToken)) === payload.session_id) {
          const refreshedJwt = await createQuestionnaireJWT(session.emailHash, session.sessionId, 'link');
          increment('auth.session.claimless_upgraded');
          return { session, via: 'link', refreshedJwt };
        }

        increment('auth.session.claimless');
        return { session, via: 'gate' };
      }
